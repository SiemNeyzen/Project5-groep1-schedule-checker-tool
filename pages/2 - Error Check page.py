...
# The function of this page is to check whether a schedule is feasible by checking for errors
...
# Since it's possible to have multiple schedules, a function is used
# This function checks the schedule for a number of errors
...
def checking_errors(dataframe):
    ...
    # For every bus, the start location of the first row and final location of the last row will be checked
    # If they aren't 'ehvgar', which represents the garage, an error has been made
    ...
    # This is checked by comparing the final location of a row with the start location of the next row
    # By removing the garage as first start- and last final location, the remaining locations can easily be compared
    # By moving up the start locations by one, comparisons can easily be made
    # Now the start- and final location of every row should be the same
    # If that isn't the case, an error has occured
    totaal_index = 0
    ...
    # For every row in the 'dienstregeling' file, it will be checked whether it's covered by each row of the schedule
    # If it's covered, the amount of it's respective position in a list will increase by 1
    # This results in a list with a 1 on every position if the schedule has no errors
    # If a 0 or value higher than 1 occurs, then an error exists in the schedule
    ...
    # For this error, a dataframe with information about the time needed to travel between locations is used
    # This dataframe contains the minimum and maximum time needed for each route
    # By comparing the time of each row with the respective minimum and maximum time, errors can be found
    ...
    for index, row in dataframe.iterrows():
        if row['activiteit'] not in ['opladen','idle']:
            ...
            # for when the clock goes from 23:.. to 00:..    
    ...
    # For every row the state of charge has to be calculated if that hasn't been done yet
    # After that, each row needs to be checked on whether the state of charge stays above 10% of the state of life
    # If that doesn't happen, an error occurs
    # Into account has to be take the state of charge value while driving different routes, being idle and while charging
    ...
    # To check this error, the difference between the start- and endtime for every row is calculated
    # If it's more than 15, an error has occured
    for index, row in dataframe.iterrows():
...
# This function creates a table with the kind of errors and the amount of times these occur
# Each error will also be explained if they occur
# Also, the rows/buses for which these errors occured will be given
def make_table(kolom2):
    ...
    if kolom2[3] > 0:
        ...
        for i in range(len(index_lijsten_voor_fouten[3][0])):
            st.write("- row " + str(index_lijsten_voor_fouten[3][0][i] + 2) + ' occupied by buses:')
    ...
    if kolom2[4] > 0:
        ...
        for i in range(len(index_lijsten_voor_fouten[4])):
            st.write("- row " + str(index_lijsten_voor_fouten[4][i]+2))
    ...
    if kolom2[5] > 0:
        ...
        for i in range(len(index_lijsten_voor_fouten[5])):
            st.write("- row " + str(index_lijsten_voor_fouten[5][i]+2))
    ...
    if kolom2[6] > 0:
        ...
        for i in range(len(index_lijsten_voor_fouten[6])):
            st.write("- row " + str(index_lijsten_voor_fouten[6][i]+2))          
    ...
    if kolom2[7] > 0:
        ...
        for i in range(len(index_lijsten_voor_fouten[7])):
            st.write("- row " + str(index_lijsten_voor_fouten[7][i]+2))



#%%
# Execute functions
# the commands below execute the functions, and design the page based on the amount of schedules given
