            ...
            if len(index_dienst) > 0:
                for index2, row2 in dataframe_zelfde_tijd.iterrows():
                    if row['startlocatie'] == row2['startlocatie']:
                        bussen_bezetting_lijst[index_dienst[index_dienst.index(index2)]].append(index)
    ...
    for index, row in dataframe.iterrows():
        if row['activiteit'] not in ['opladen','idle']:
            ...
            if row['omloop nummer'] not in busnummers:
                ...
                if row['state of charge'] == 350: #als de SoC lager dan 350 is, gaan we ervan uit dat het verbruik van de eerste rit al meegenomen is
                    dataframe.at[index, 'state of charge'] = 350 - verbruik
            else:
            ...
            if row['activiteit'] == 'idle':
                ...
                kwh_tot_90_procent = 315 - float(dataframe.iloc[index-1]['state of charge'])
                ...
                if tijd_tot_90_procent > tijd:
                    ...
                    opladen = tijd/60 * 250
                if float(dataframe.iloc[index-1]['state of charge']) + opladen < 350:
                    dataframe.at[index, 'state of charge'] = float(dataframe.iloc[index-1]['state of charge']) + opladen
                else:
                    dataframe.at[index, 'state of charge'] = 350

    for index, row in dataframe.iterrows():
        if row['state of charge'] < 35:
            kolom2[6] += 1
...
def make_table(kolom2):
    ...
    if kolom2[2] > 0:
        ...
        for i in range(len(index_lijsten_voor_fouten[2])):
            st.write("- row " + str(index_lijsten_voor_fouten[2][i] + 1))
    ...
    if kolom2[3] > 0:
        ...
        for i in range(len(index_lijsten_voor_fouten[3][0])):
            st.write("- row" + str(index_lijsten_voor_fouten[3][i] + 1) + ' occupied by buses:')
            for j in range(len(index_lijsten_voor_fouten[3][1])):
                st.write("- bus " + str(index_lijsten_voor_fouten[3][j]))
    ...
    if kolom2[6] > 0:
        st.write("Error 6 (A bus is below the desired minimum state of charge while still driving) occured at least once. "
                 "This means that a bus is driving while it should be charging, which is when its State of Charge is below 10%. "
