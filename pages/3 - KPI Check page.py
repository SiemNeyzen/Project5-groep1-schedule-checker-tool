...
# This page shows the user the performance of the schedule(s)
...
# The function below is made since the performance might need to be measured twice
# It performs the necessary steps to calculate the performance
def measure_performance(dataframe):
    kolom2 = [0,0,0]
    kolom2[0] = dataframe['omloop nummer'].nunique()
    
# To calculate the DD value, the difference between the total time and time as bus service need to be calculated
# The time of each row will be summed, while the time of bus services will also be summed
# The total time will be divided by the total time as bus service, which is the DD value
    totaaltijd = 0
    diensttijd = 0
...
# To calculate the kWh that can be given back to the network, the state of charge of each row has to be calculated first
# This code is mostly reused code of the error about the state of charge
# During that, calculations will be made about how many kWh can be given back to the network
# This is done by checking the state of charge of the first row of each bus
        ...
        if row['activiteit'] not in ['opladen','idle']:
            ...
            if row['omloop nummer'] not in busnummers:
                ...
                if row['state of charge'] == 350*StateOfHealth_capaciteit: 
                    ...
                    bespaarde_hoeveelheid += (350*StateOfHealth_capaciteit) - verbruik - row['state of charge']
            else:
...
# There is a difference in output based on whether there are 1 or 2 schedules
# 2 schedules mean that charts are meaningful to make, while that isn't the case for 1 schedule
# In either case, a table will be made and an explanation about the performances will be given
# The user is also given an explanation about the meaning of the KPI's
